...
const { stick, errorfg, group, banf, ownerB, modsB, userB, admin, Badmin, sofii, mariann, samm, shagii, garyy, criss, pandaa, yulii, miaa, isaa, famp, modee} = require('./help/respon');
...
const vcard1 = 'BEGIN:VCARD\n' 
            ...
            + 'FN:𝑖𝑠𝑎𝑖𝑎𝑠𝑤𝑡𝑓\n' 
            + 'ORG:•𝑪𝒉𝒂𝒓𝒍𝒐𝒕𝒕𝒆ᶤ🌹;\n'
             + `TEL;type=CELL;type=VOICE;waid=${isai}:+${isai}\n`
            + 'END:VCARD' 
...
if(budy.includes('bot')) {cnf.sendMessage(from, 'Hola tqm', MessageType.text, {quoted: mek})}
...
if(budy.includes('pito')) {cnf.sendMessage(from, 'Donde?', MessageType.text, {quoted: mek})}
...
if(budy.includes('jalo')) {cnf.sendMessage(from, 'Pit0s', MessageType.text, {quoted: mek})}
if(budy.includes('mía')) {cnf.sendMessage(from, 'Mía no se baña', MessageType.text, {quoted: mek})}
if(budy.includes('Mía')) {cnf.sendMessage(from, 'Mía no se baña', MessageType.text, {quoted: mek})}
//--Auto respon media
...
txtpanda=`*Arre, ahí le va su admin compa 🐎*`
...
txteddy=`*Que pasa bro, toma tu admin*\n*Y que chsm el fantasma*`
...
case 'send':
cnf.sendMessage(`${sender.split('@')[0]}@s.whatsapp.net`, `Realizado`, MessageType.text)
break
...
*•Numero : @${stod.split('@')[0]}*`,
 sections: [
                     {
                      "title": `•𝑪𝒉𝒂𝒓𝒍𝒐𝒕𝒕𝒆ᶤ🌹 ᵇᵒᵗ`, "body": `𝓑𝓸𝓽 𝓫𝔂 𝓲𝓼𝓪𝓲𝓪𝓼𝔀𝓽𝓯`,
 rows: [
                           {
                              "title": "🌿Estado",
                           ...
                           {
                              "title": "🎲MenuJuegos",
                              "description": "Se implementarán mas juegos :D",
                              "rowId": ""
                           },
                           {
                              "title": "🔥MenuNSFW",
...
*◈​​┃${prefix}fb [ link ]*
*◈​​└${prefix}twitter [ link ]*`
cnf.sendMessage(from, menu, text, { quoted: mek, thumbnail: fs.readFileSync('./confumods.jpg'), contextInfo: { externalAdReply:{title: `𝗠𝗘𝗡𝗨 𝗗𝗘𝗦𝗖𝗔𝗥𝗚𝗔𝗦\n`,body:"𝓑𝓸𝓽 𝓫𝔂 𝓲𝓼𝓪𝓲𝓪𝓼𝔀𝓽𝓯",mediaType:"2",thumbnail:ofrply,sourceUrl:`https://wa.me/${sender.split("@")[0]}`}}})
break
case 'makermenu':
case 'creadormenu':
case '🎨menucrear':
...
sendButLocation(from, `${menu}`, `*Solicitado por:* @${stod.split('@')[0]}\n*Rango:* ${tipoDeUsr}\nHora : ${timeWib}\nTiempo : ${tampilUcapan}\nFecha : ${calender}\n\n\n𝓑𝓸𝓽 𝓫𝔂 𝓲𝓼𝓪𝓲𝓪𝓼𝔀𝓽𝓯`, {jpegThumbnail:ofrply}, [{buttonId:`sam`,buttonText:{displayText:'🔥 SEX TIME'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
} else if (isSofi) {
...
sendButLocation(from, `${menu}`, `*Solicitado por:* @${stod.split('@')[0]}\n*Rango:* ${tipoDeUsr}\nHora : ${timeWib}\nTiempo : ${tampilUcapan}\nFecha : ${calender}\n\n\n𝓑𝓸𝓽 𝓫𝔂 𝓲𝓼𝓪𝓲𝓪𝓼𝔀𝓽𝓯`, {jpegThumbnail:ofrply}, [{buttonId:`panda`,buttonText:{displayText:'🐎 JALESE PLEBE'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
} else if (isYuli) {
sendButLocation(from, `${menu}`, `*Solicitado por:* @${stod.split('@')[0]}\n*Rango:* ${tipoDeUsr}\nHora : ${timeWib}\nTiempo : ${tampilUcapan}\nFecha : ${calender}\n\n\n𝓑𝓸𝓽 𝓫𝔂 𝓲𝓼𝓪𝓲𝓪𝓼𝔀𝓽𝓯`, {jpegThumbnail:ofrply}, [{buttonId:`yuli`,buttonText:{displayText:'🐧 ADMIN PA YULI'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
} else if (isEddy) {
sendButLocation(from, `${menu}`, `*Solicitado por:* @${stod.split('@')[0]}\n*Rango:* ${tipoDeUsr}\nHora : ${timeWib}\nTiempo : ${tampilUcapan}\nFecha : ${calender}\n\n\n𝓑𝓸𝓽 𝓫𝔂 𝓲𝓼𝓪𝓲𝓪𝓼𝔀𝓽𝓯`, {jpegThumbnail:ofrply}, [{buttonId:`eddy`,buttonText:{displayText:'☠️ AUTOADMIN'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
} else if (isMods) {
...
case "mp3":
if (args.length === 0) return reply(`Escribe *${prefix}play* _El tÃ­tulo de la canciÃ³n que se buscarÃ¡_`);
var srch = args.join("");
...
Turno de = @${tty.er1.split('@')[0]}`
 cnf.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.er1,tty.er2]}})
     ...
     cnf.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `──「 *ALERTA-DE-ERROR* 」──\n\n\`\`\`${e}\`\`\`\n\n──────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Desarrollador de Charlotte",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./confumods.jpg'),sourceUrl:"https://youtube.com/c/ConfuMods"}}})
	}
    }
    }
    }
